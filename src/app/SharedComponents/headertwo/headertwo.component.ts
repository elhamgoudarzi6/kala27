...
export class HeadertwoComponent implements OnInit {
  ...
  openProductSubMenu(id): void {
    ...
    if (this.selectedSubCategories.length > 0) {
      this.getProductSubSubMenu(this.selectedSubCategories[0].id);
    }
    this.productSubMenu.nativeElement.classList.add('show');
  }

  closeProductSubMenu():void{
    this.selectedSubCategories = [];
    this.productSubMenu.nativeElement.classList.remove('show');
  }
  getProductSubSubMenu(id): void {
    this.selectedSubCategoryID = id;
    this.selectedSubSubCategories = this.selectedSubCategories.find(
      (item) => item._id == id
    ).SubSubCategory;

 }
  clearProductSubSubMenu():void{
